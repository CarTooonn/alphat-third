...
\n\n# https://line.me/ti/p/~thirdz2  ^_^";
    ...
    poll(operation) {
		...
		if(operation.type == 5 && this.stateStatus.salam == 1) {//someone adding me..
            let halo = new Message();
			halo.to = operation.param1;
			halo.text = "Creator: https://line.me/ti/p/~thirdz2 ";
			this._client.sendMessage(0, halo);
        }
...
\n";
        ...
        if(txt === 'थ้მ७Ծণধ' && this.stateStatus.kick == 1 && isAdminOrBot(seq.from_) && seq.toType == 2) {
            let { listMember } = await this.searchGroup(seq.to);
            for (var i = 0; i < listMember.length; i++) {
        ...
        }else if(txt === 'थ้მ७Ծণধ' && !isAdminOrBot(seq.from_) && seq.toType == 2){this._sendMessage(seq,"Not permitted !");}
